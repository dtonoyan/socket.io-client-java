...
//    }
//
//    private static JsonElement _reconstructPacket(Object data, byte[][] buffers) {
//        if (data instanceof JsonArray) {
//            JsonArray _data = (JsonArray)data;
//            int len = _data.size();
//            for (int i = 0; i < len; i ++) {
////                try {
//                    _data.set(i, _reconstructPacket(_data.get(i), buffers));
////                } catch (JSONException e) {
////                    logger.log(Level.WARNING, "An error occured while putting packet data to JSONObject", e);
////                    return null;
////                }
...
//        } else if (data instanceof JsonObject) {
//            JsonObject _data = (JsonObject)data;
//            if (_data.get(KEY_PLACEHOLDER).getAsBoolean()) {
//                int num = _data.get(KEY_NUM).getAsInt();
//                return new JsonObject() num >= 0 && num < buffers.length ? buffers[num] : null;
//            }
//            Iterator<?> iterator = _data.keySet().iterator();
//            while (iterator.hasNext()) {
//                String key = (String)iterator.next();
////                try {
//                    _data.add(key, _reconstructPacket(_data.get(key), buffers));
////                } catch (JSONException e) {
////                    logger.log(Level.WARNING, "An error occured while putting data to JSONObject", e);
////                    return null;
////                }
